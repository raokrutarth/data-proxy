...
_DATA_QUEUES: persistqueue.PDict = persistqueue.PDict(
...
def _get_queue_details(queue_id: str) -> Tuple[str, str]:
    if queue_id in _DATA_QUEUES:
        queue_name, queue_path = _DATA_QUEUES[queue_id]
...
def _add_to_queue(queue_id: str, payload: dict):
    ...
    q_name, q_path = _get_queue_details(queue_id)

    if not q_name:
        # queue doesn't exist
        log.info(f"Creating new queue. Data map held in directory: {_DATA_QUEUES.path}")
        q_name = f"generic_proxy_queue_{str(uuid1())}"
        q_path = join(_DATA_QUEUES.path, q_name)
        _DATA_QUEUES[queue_id] = (q_name, q_path)
        log.info(
...
def get_latest_event(
    ...
    log.info(f"Sending latest payload from queue {queue_id} to user {username}")
